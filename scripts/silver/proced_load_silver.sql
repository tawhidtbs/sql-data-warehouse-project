...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO silver.crm_prd_info(
		  ...
		  SELECT 
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id,		-- Seperating the product key into category id by extracting the product key 
			SUBSTRING(prd_key, 7, LEN(prd_key)) AS prd_key,				-- Seperating the product key into sales order id by extracting the product key 
			prd_nm,
			...
			CAST(
				...
				) AS prd_end_dt											-- Calculate end date as ond day before the next start date
			...
			INSERT INTO silver.erp_cust_az12(
				...
				SELECT
					...
					CASE
						...
						ELSE 'Unknown'
					END gen							-- Normalize gender value and handle unknown cases
